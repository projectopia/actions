...
const listOfInputsToExclude = core
  .getInput("exclude_inputs")
  .split(",")
  .map((item) => item.trim());
...
core.info(
  `${colors.blue.open}List of inputs to exclude: ${listOfInputsToExclude}`
);
...
core.info(
  `${colors.yellow.open}After removing the inputs to exclude, these are all of the inputs that will be hidden: ${inputKeysToHide}`
);
